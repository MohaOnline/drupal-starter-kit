...
namespace OpenlayersDrupal\Component\Utility;
...
class Unicode {
   ...
   * Boundaries algorithm described in http://unicode.org/reports/tr29, as
   * PCRE does not contain the Word_Break property table, this simpler
   * algorithm has to do.
...
EOD;
   ...
   *   - \OpenlayersDrupal\Component\Utility\Unicode::STATUS_MULTIBYTE
   *     Full unicode support using an extension.
   *   - \OpenlayersDrupal\Component\Utility\Unicode::STATUS_SINGLEBYTE
   *     Standard PHP (emulated) unicode support.
   *   - \OpenlayersDrupal\Component\Utility\Unicode::STATUS_ERROR
  ...
  public static function strtoupper($text) {
    if (static::getStatus() == static::STATUS_MULTIBYTE) {
      ...
      $text = preg_replace_callback('/\xC3[\xA0-\xB6\xB8-\xBE]/', '\OpenlayersDrupal\Component\Utility\Unicode::caseFlip', $text);
      return $text;
    }
  }
  ...
  public static function strtolower($text) {
    if (static::getStatus() == static::STATUS_MULTIBYTE) {
      ...
      // Case flip Latin-1 accented letters.
      $text = preg_replace_callback('/\xC3[\x80-\x96\x98-\x9E]/', '\OpenlayersDrupal\Component\Utility\Unicode::caseFlip', $text);
      return $text;
    }
  }
  ...
  public static function substr($text, $start, $length = NULL) {
    if (static::getStatus() == static::STATUS_MULTIBYTE) {
      ...
      elseif ($length > 0) {
        ...
        // have found $length characters or reached the end of the string,
        // then backtrace one byte.
        $iend = $istart - 1;
        $chars = -1;
        $last_real = FALSE;
   ...
   *   An upper limit on the returned string length, including trailing
   *   ellipsis if $add_ellipsis is TRUE.
   * @param bool $wordsafe
   ...
   *   parameters $max_length and $min_wordsafe_length), word boundaries
   *   are ignored.
   ...
   *   10, though, the function would realise that "See" alone is too short,
   *   and would then just truncate ignoring word
