...
use OpenlayersDrupal\Component\Plugin\Exception\InvalidDeriverException;

/**
 * FIX - insert comment here.
 *
 * Base class providing the tools for a plugin discovery to be derivative
 * aware.
 *
...
class DerivativeDiscoveryDecorator implements DiscoveryInterface {
   ...
   * Plugin derivers.
   *
   * @var \OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface[]
   ...
   * {@inheritdoc}
   *
   * @throws \OpenlayersDrupal\Component\Plugin\Exception\InvalidDeriverException
   *   Thrown if the 'deriver' class specified in the plugin definition
   *   does not implement
   *   \OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface.
   */
   ...
   *
   * @return \OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface|null
   ...
   *   \OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface.
   */
  protected function getDeriver($base_plugin_id, array $base_definition) {
   ...
   *   Thrown if the 'deriver' class specified in the plugin definition
   *   does not implement
   *   \OpenlayersDrupal\Component\Plugin\Derivative\DerivativeInterface.
   */
  protected function getDeriverClass(array $base_definition) {
    $class = NULL;
    if ((is_array($base_definition) || ($base_definition = (array) $base_definition)) && (isset($base_definition['deriver']) && $class = $base_definition['deriver'])) {
      ...
      if (!is_subclass_of($class, '\OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface')) {
        throw new InvalidDeriverException(sprintf('Plugin (%s) deriver "%s" must implement \OpenlayersDrupal\Component\Plugin\Derivative\DeriverInterface.', $base_definition['id'], $class));
      }
    }
    return $class;
  }
   ...
   */
  protected function mergeDerivativeDefinition(array $base_plugin_definition, array $derivative_definition) {
