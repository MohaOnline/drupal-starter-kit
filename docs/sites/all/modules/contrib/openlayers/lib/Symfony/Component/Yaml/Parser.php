...
class Parser {
  ...
  public function parse($value, $exceptionOnInvalidType = FALSE, $objectSupport = FALSE, $objectForMap = FALSE) {
    ...
    while ($this->moveToNextLine()) {
      ...
      elseif (preg_match(
        '#^(?P<key>' . Inline::REGEX_QUOTED_STRING . '|[^ \'"\[\{].*?) *\:(\s+(?P<value>.+?))?\s*$#u',
        $this->currentLine,
        $values
        ...
        if ('<<' === $key) {
          ...
          if (isset($values['value']) && 0 === strpos($values['value'], '*')) {
            ...
            if (isset($parsed[0])) {
              // If the value associated with the merge key is a sequence,
              // then this sequence is expected to contain mapping nodes
              // and each of these nodes is merged in turn according to its
              // order in the sequence. Keys in mapping nodes earlier
              ...
              // each of its key/value pairs is inserted into the
        ...
        elseif (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
          // If next line is less indented or equal, then it means that the
          // current value is null.
          if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
            // Spec: Keys MUST be unique; first one wins.
            // But overwriting is allowed when a merge node is used in
            // current block.
        ...
        // A 1-liner optionally followed by newline(s).
  ...
  private function getNextEmbedBlock($indentation = NULL, $inSequence = FALSE) {
    ...
    if ($inSequence && $oldLineIndentation === $newIndent && '-' === $data[0][0]) {
      // The previous line contained a dash but no item content, this line is
      // a sequence item with the same indentation
    ...
    // Comments must not be removed inside a string block (ie. after a line
    // ending with "|").
  ...
  private function isCurrentLineComment() {
    // Checking explicitly the first char of the trim is faster than loops or
    // strpos.
