...
use Drupal\openlayers\DependencyInjection\Container;
 ...
 * @codeCoverageIgnore
 *
 * FIX The alter and plugin_manager_parts are unit testable.
 */
class ServiceContainerServiceProvider implements ServiceProviderInterface {
  ...
  public function getContainerDefinition() {
    ...
    $parameters['service_container.plugin_manager_types'] = array(
      'ctools' => '\Drupal\openlayers\Plugin\Discovery\CToolsPluginDiscovery',
    );

    $services = array();

    $services['drupal7'] = array(
      'class' => 'Drupal\openlayers\Legacy\Drupal7',
    );
    ...
    $services['module_handler'] = array(
      'class' => '\Drupal\openlayers\Extension\ModuleHandler',
      'arguments' => array(DRUPAL_ROOT, '@drupal7'),
    );

    $services['module_installer'] = array(
      'class' => '\Drupal\openlayers\Extension\ModuleInstaller',
      'arguments' => array('@drupal7'),
    );

    $services['database'] = array(
      'class' => 'OpenlayersDrupal\Core\Database\Connection',
      'factory_class' => 'OpenlayersDrupal\Core\Database\Database',
      'factory_method' => 'getConnection',
      'arguments' => array('default'),
    );

    $services['flood'] = array(
      'class' => '\Drupal\openlayers\Flood\LegacyBackend',
    ...
    $services['state'] = array(
      'class' => 'OpenlayersDrupal\Core\State\State',
      'arguments' => array('@keyvalue'),
    );

    $services['variable'] = array(
      'class' => 'Drupal\openlayers\Variable',
      'arguments' => array('@drupal7'),
    );

    $services['lock'] = array(
      'class' => 'OpenlayersDrupal\Core\Lock\DatabaseLockBackend',
    ...
    $services['lock.persistent'] = array(
      'class' => 'OpenlayersDrupal\Core\Lock\PersistentDatabaseLockBackend',
      'arguments' => array('@database'),
      'tags' => array(
        array('name' => 'backend_overridable'),
      ),
    ...
    $services['messenger'] = array(
      'class' => 'Drupal\openlayers\Messenger\LegacyMessenger',
      'arguments' => array('@drupal7'),
    );

    $services['url_generator'] = array(
      'class' => 'Drupal\openlayers\UrlGenerator',
      'arguments' => array('@drupal7'),
    );

    $services['link_generator'] = array(
      'class' => 'Drupal\openlayers\LinkGenerator',
      'arguments' => array('@drupal7'),
    );

    $services['current_user'] = array(
      'class' => 'Drupal\openlayers\Session\Account',
      'arguments' => array('@drupal7', '@variable'),
    );

    // Logging integration.
    $services['logger.factory'] = array(
      'class' => 'Drupal\openlayers\Logger\LoggerChannelFactory',
      'calls' => array(
    ...
    $services['logger.dblog'] = array(
      'class' => 'Drupal\openlayers\Logger\WatchdogLogger',
      'arguments' => array('@drupal7'),
      'tags' => array(
        array('name' => 'logger'),
      ),
    ...
    $services['logger.channel.default'] = array(
      'class' => 'Drupal\openlayers\Logger\LoggerChannel',
  ...
  public function alterContainerDefinition(array &$container_definition) {
    ...
    if (!empty($container_definition['parameters']['annotated_plugins_auto_discovery'])) {
   ...
   * @param array $parameter_definitions
   *   The parameter definition.
   */
  public function registerAnnotatedPluginTypes(array &$container_definition, array $parameter_definitions) {
   ...
   */
  public function registerCtoolsPluginTypes(array &$container_definition, array $ctools_types) {
   ...
   *   The type of plugin.
   */
  public function registerAliasServices(array &$container_definition, $owner, $plugin_type) {
  ...
  /**
   * Processes plugin managers of varying types.
   *
   ...
   */
  public function processPluginManagers(array &$container_definition, $discovery_class, array $plugin_managers) {
   ...
   *   FIX.
   */
  protected function getPluginManagerDefinition($name, $discovery_class, array $plugin_manager) {
  ...
  /**
   * Return the full list of plugin types.
   *
   ...
   * used as the way to initially populate the list of plugin types. Make sure
   * you call this function to properly populate the ctools_plugin_type_info
   * static variable.
   *
   * @return array
