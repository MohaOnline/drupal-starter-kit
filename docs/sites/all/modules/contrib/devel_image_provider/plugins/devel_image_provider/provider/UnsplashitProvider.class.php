 ...
 */
class UnsplashitProvider extends DevelImagesProviderBase {
  ...
  public function generateImage($object, $field, $instance, $bundle) {
    ...
    if (!isset($images[$extension][$min_resolution][$max_resolution]) || count($images[$extension][$min_resolution][$max_resolution]) <= DEVEL_GENERATE_IMAGE_MAX) {
      if ($tmp_file = drupal_tempnam('temporary://', 'imagefield_')) {
        ...
        $blurred = isset($this->settings['devel_image_provider_tags']) ? $this->settings['devel_image_provider_tags'] : NULL;
        $blurred_param = $blurred ? '&blur' : '';

        $url = "$this->provider_base_url" . $gray_part . "/$width/$height?random" . $blurred_param;
