 ...
 */
function _wetkit_bootstrap_book_children($book_link) {
  ...
  _wetkit_bootstrap_book_fix_theme_hooks($book_link['nid'], $tree);
 ...
 */
function _wetkit_bootstrap_book_fix_theme_hooks($bid, array &$element, $level = 0) {
  ...
  foreach (element_children($element) as $child) {
    ...
    if (!empty($element[$child]['#below'])) {
      _wetkit_bootstrap_book_fix_theme_hooks($bid, $element[$child]['#below'], ($level + 1));
    }
  }
}
