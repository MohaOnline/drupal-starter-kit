...
Drupal.behaviors.quizQuestionBrowserBehavior = {
  attach: function(context, settings) {
    ...
    if ($('#question-list', context).length && typeof settings.newest_question != 'undefined' && settings.newest_question != '') {
    ...
    .click(function(event) {
      ...
      // Questions will be loaded using AJAX. We need just to check items as selected.
      $('.quiz-browser-checkbox').each(function() {
        if (!ch) {
          $(this).not(':not(:checked)').click();
        }
        else {
          $(this).not(':checked').click();
        }
      });
    });
