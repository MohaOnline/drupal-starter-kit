...
class OgSelectionHandler extends EntityReference_SelectionHandler_Generic {
  ...
  public function buildEntityFieldQuery($match = NULL, $match_operator = 'CONTAINS') {
    ...
    $user_groups = og_get_groups_by_user($this->account, $group_type);
    ...
    if ($field_mode == 'default') {
      if ($user_groups && !empty($this->instance) && $this->instance['entity_type'] == 'node') {
        ...
        foreach ($user_groups as $gid) {
          ...
          if (og_user_access($group_type, $gid, "create $node_type content", $this->account)) {
            $ids[] = $gid;
          }
          elseif (!empty($node->nid) && (og_user_access($group_type, $gid, "update any $node_type content", $this->account) || ($user->uid == $node->uid && og_user_access($group_type, $gid, "update own $node_type content", $this->account)))) {
  ...
  /**
   * Set the $account that should be used for all access checks.
   * By default (or if $account is set to NULL) all access checks are done using
   * the global $user.
   *
   * @return
   *   The handler object (so that function can be chained)
   */
  ...
  private function getGidsForCreate() {
    ...
    foreach ($ids as $delta => $id) {
      if (!is_numeric($id) || !$id || !og_user_access($this->field['settings']['target_type'], $id, "create $node_type content", $this->account)) {
        unset($ids[$delta]);
      }
    }
    return $ids;
