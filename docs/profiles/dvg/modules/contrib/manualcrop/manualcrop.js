  ...
  ManualCrop.isLoaded('.manualcrop-file-' + fid + '-holder', function() {
    ...
    if (!ManualCrop.croptool) {
      ...
      if (typeof styleSelect != 'undefined') {
        ...
        $('.manualcrop-style-button-' + fid + '-' + styleName).hide();
      }
...
ManualCrop.selectionStored = function(element, fid, styleName) {
  ...
  ManualCrop.isLoaded('.manualcrop-file-' + fid + '-holder', function() {
    var previewHolder = $(element).closest(':has(.manualcrop-preview-' + fid + '-' + styleName + ')').find('.manualcrop-preview-' + fid + '-' + styleName + ' .manualcrop-preview-cropped');
    if (!previewHolder.length) {
      previewHolder = $(element).closest(':has(.manualcrop-preview-' + fid + ')').find('.manualcrop-preview-' + fid + ' .manualcrop-preview-cropped');
    }

    var defaultPreview = $(element).closest(':has(.manualcrop-preview-' + fid + '-' + styleName + ')').find('.manualcrop-preview-' + fid + '-' + styleName + ' > img');
    if (!defaultPreview.length) {
      defaultPreview = $(element).closest(':has(.manualcrop-preview-' + fid + ')').find('.manualcrop-preview-' + fid + ' > img');
    }

    // Change the elements if Media is detected.
    var media = $(element).closest(':has(.manualcrop-preview-' + fid + ')').find('.manualcrop-preview-' + fid + ' .media-item[data-fid] .media-thumbnail');

    if (media.length) {
      media.prepend(previewHolder);
      previewHolder = media.find('.manualcrop-preview-cropped');
      defaultPreview = media.children('img');
    }

    var toolOpener = $('.manualcrop-style-select-' + fid + " option[value='" + styleName + "'], .manualcrop-style-button-" + fid + '-' + styleName + ', .manualcrop-style-thumb-' + fid + '-' + styleName + ' .manualcrop-style-thumb-label');
    ...
    if (selection) {
      if (previewHolder.length) {
        ...
        if (maxWidth > 0) {
          ...
          var originalImage = previewHolder.closest(':has(#manualcrop-overlay-' + fid + ', #manualcrop-inline-' + fid + ')').find('#manualcrop-overlay-' + fid + ' img.manualcrop-image, #manualcrop-inline-' + fid + ' img.manualcrop-image');
